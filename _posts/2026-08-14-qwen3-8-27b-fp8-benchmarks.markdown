...
4. **Thinking depth** — the four reasoning levels (xhigh, medium, low, off), measured on both quality and speed. This is the dial that trades reasoning tokens against latency, so it gets its own measurements: BenchLocal scores plus TTFT and token generation rate per level.
